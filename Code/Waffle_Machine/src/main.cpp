// Waffle Machine code
// For Einsy Rambo 1.2b
// Board: Arduino MEGA

// Written by waffle group 2022.
#include <Arduino.h>
#include <TMCStepper.h> // from library manager
#include <Servo.h>
...
#define STEP_X_PIN 37    // Step pin X-motor
...
#define R_SENSE 0.2f // R_sense resistor for einsy
...
TMC2130Stepper X_DRIVER(CS_X_PIN, R_SENSE, SW_MOSI, SW_MISO, SW_SCK);

// STATES
#define STANDBY 0
#define OPEN_IRON 1
#define MOVE_TUBE_TO_POSITION 2
#define DISPENSE_WAFFLE_MIXTURE 3
#define MOVE_TUBE_BACK_TO_POSITION 4
#define CLOSE_IRON 5
#define COOK_WAFFLE 6
#define WAFFLE_DONE 7
#define OPEN_IRON_DONE 8
#define DISPENSE_WAFFLE 9
#define EMERGENCY 10
#define CUSTOM 11
int currentState = CUSTOM;
...
void setup()
{
  ...
  // Button setup
  pinMode(MAKE_WAFFLE_PIN, INPUT_PULLUP);
...
*/
  ...
  // New Setup X-motor-driver
  X_DRIVER.begin();
  X_DRIVER.toff(5);
  X_DRIVER.rms_current(600);
  X_DRIVER.microsteps(16);
...
void loop()
{
  ...
  case CUSTOM:
  {
    ...
    X_DRIVER.shaft(shaft);
    delay(1000);
    digitalWrite(DIR_X_PIN, shaft);
    break;
  }
  ...
  case STANDBY:
  {
    ...
    if (digitalRead(MAKE_WAFFLE_PIN))
    { // TODO: check HIGH/LOW IS CORRECT
  ...
  case OPEN_IRON:
  {
    ...
    currentState = MOVE_TUBE_TO_POSITION;
    break;
  }
...
bool getButtonFallingEdge(int pin) {
  bool fallingEdge = false;
  int newButtonInput = digitalRead(pin);
